...
if($akcja=="nowy"){
	...
	$rok = date("Y");
	$sql = "SELECT `katnews_id` FROM `kategorie_news` 
			ORDER BY `katnews_id` DESC LIMIT 1";
	$query = mysqli_query($link, $sql);
	$dane = mysqli_fetch_array($query);
	$id = $dane['katnews_id'] + 1;
	$lokalizacja = "./zasoby/".$rok.
			"/news_kategorie/news_kategoria".$id.".jpg";
	...
	$sql = "SELECT * FROM `kategorie_news` 
	...
	echo'
		<form enctype="multipart/form-data" 
			action="index.php?akcja=edytuj&id='.
			$_GET['id'].'" method="post">
		<label>Nazwa kategorii newsów:
		<input name="nazwa" type="text" size="30"
		...
		<label>Kolejność:
		<input name="kolejnosc" type="text" size="3"
			value="'.$rekord['katnews_kolejnosc'].'">
		</label><br>
	...
	if(!$query){
		echo "Wystąpił błąd 
				przy zmianie kategorii ".$_GET["id"];
	} else {
	...
	if(!$query){
		echo "Wystąpił błąd 
				przy usuwaniu kategorii ".$_GET["id"];
	} else {
	...
	$sql = "SELECT * FROM `kategorie_news` WHERE 1 
				ORDER BY `katnews_kolejnosc` ASC";
